...
class InventariosController extends Controller
{
    ...
    public function createinventario(Request $request)
    {
        ...
        } else if (strlen($request->codigoUbicacion) === 10) {
            // Nivel 5
            $codigoUbicacionN5 = $request->codigoUbicacion;
        } else if (strlen($request->codigoUbicacion) === 12) {
            // Nivel 6
            $codigoUbicacionN6 = $request->codigoUbicacion;
        }
        ...
        $inventario->codigoUbicacionN5   = $codigoUbicacionN5 ?? 0;
        $inventario->codigoUbicacionN6   = $codigoUbicacionN6 ?? 0;
    ...
    public function deleteImageByEtiqueta($etiqueta, $id_img, $idLista, $idActivo = null, $cycleid)
    {
        ...
        $thumbName = ImageService::buildThumbnailName($imagenCrud->picture);
        $thumbPath = PictureSafinService::getImgSubdir(Auth::user()->nombre_cliente) . '/' . $thumbName;
        ...
        DB::table('crud_imagenes_thumbnails')
            ->where('id_img', $imagenCrud->id_foto)
            ->where('etiqueta', $etiqueta)
            ->where('picture', $thumbName)
            ->delete();
    ...
    public function addImageByEtiqueta(Request $request, $etiqueta)
    {
        ...
        try {
            ...
            foreach ($request->file('imagenes') as $file) {
                ...
                if ($esCrudActivo) {
                    $idFoto = DB::table('crud_activos_pictures')->insertGetId([
                        'id_activo'    => $idActivo,
                        'picture'      => $filename,
                        'origen'       => $origen,
                    ...
                    $thumbUrl = $thumbnail?->url_imagen ?? ImageService::getCrudThumbnailUrlByPicture($idFoto, $etiqueta, $filename);
                } else {
                    ...
                    $thumbnail = ImageService::createThumbnail($file, $request->user()->nombre_cliente, $filename);
                    $thumbUrl = $thumbnail?->url_imagen ?? ImageService::getInventoryThumbnailUrlByPicture($id_img, $etiqueta, $filename);
                }
                ...
                $paths[] = [
                    ...
                    'url'      => $url,
                    'filename' => $filename,
                    'original_url' => ImageService::buildOriginalUrl($url, $url_pict, $filename),
    ...
    public function storeInventoryMultiple(int $ciclo, Request $request)
    {
        ...
        // $request->merge(['emplazamientoN1' => json_encode($request->emplazamientoN1)]);
        // $request->merge(['emplazamientoN2' => json_encode($request->emplazamientoN2)]);
        // $request->merge(['emplazamientoN3' => json_encode($request->emplazamientoN3)]);
        ...
        [$mapaIdN5, $mapaCodN5] = $this->procesarUbicacionesNn($request->emplazamientoN4, 5, $ciclo, $usuario, $idMapaGeo, $mapaCodN4, $id_proyecto);
        [$mapaIdN6, $mapaCodN6] = $this->procesarUbicacionesNn($request->emplazamientoN4, 6, $ciclo, $usuario, $idMapaGeo, $mapaCodN5, $id_proyecto);
        // return response()->json([
        //     'idMapaGeo' => $idMapaGeo,
    ...
    private function mapActivo(
        ...
        return [
            ...
            'idUbicacionN2'      => isset($mapaIdN2[$item->codigoUbicacion_N2]) ? $mapaIdN2[$item->codigoUbicacion_N2] : ($item->idUbicacionN2 ?? '0'),
            ...
            'codigoUbicacionN5'  => isset($mapaCodN4[$item->codigoUbicacionN5]) ? $mapaCodN4[$item->codigoUbicacionN5] : ($item->codigoUbicacionN5 ?? '0'),
            'codigoUbicacionN6'  => isset($mapaCodN4[$item->codigoUbicacionN6]) ? $mapaCodN4[$item->codigoUbicacionN6] : ($item->codigoUbicacionN6 ?? '0'),
            'responsable'        => $responsable,
            'idResponsable'      => $getIdResponsable,
     ...
     * 
     * @deprecated replace by procesarUbicacionesNn
     */
    private function procesarUbicacionesN2($json, int $ciclo, string $usuario, array $idMapaGeo, array $idMapaN1_Codigo, $id_proyecto): array
    {
        $data = $json ? json_decode($json) : [];
        $mapaId = [];
        $mapaCodigo = [];
        ...
        foreach ($data as $n2) {
            ...
            $newN1 = $idMapaN1_Codigo[$prefijo];


            // Si el prefijo del código N2 coincide con el nuevo código N1, entonces se verifica la existencia del código N2
            if ($prefijo !== $newN1) {
                $prefijo = $newN1;
            }
            ...
            //Nuevo Código N2
            $codigoExistente = $codigoExistente ?? '0000';
            $codigoExistente = str_pad($codigoExistente, 4, '0', STR_PAD_LEFT);
            $codn3 = (int)substr($codigoExistente, 2, 2) + 1;
            ...
            if (!$registro) {
                ...
                $mapaId[$n2->codigoUbicacion] = $idInsertado;
                $mapaCodigo[$n2->codigoUbicacion] = $nuevoCodigo;
            } else {
                $mapaId[$n2->codigoUbicacion] = $registro->idUbicacionN2;
                $mapaCodigo[$n2->codigoUbicacion] = $registro->codigoUbicacion;
            }
        }
     ...
     * @deprecated replace by procesarUbicacionesNn
     */
    private function procesarUbicacionesN3($json, int $ciclo, string $usuario, array $idMapaGeo, array $idMapaN2_Codigo, $id_proyecto): array
    {
        $data = $json ? json_decode($json) : [];
        $mapaId = [];
        $mapaCodigo = [];
        ...
        foreach ($data as $n3) {
            ...
            //Nuevo Código N3
            $codigoExistente = $codigoExistente ?? '000000';
            ...
            if (!$registro) {
                ...
                $mapaId[$n3->codigoUbicacion] = $idInsertado;
                $mapaCodigo[$n3->codigoUbicacion] = $nuevoCodigo;
            } else {
    ...
    private function procesarUbicacionesNn($json, int $level, int $ciclo, string $usuario, array $idMapaGeo, array $idMapaN_minus_1_Codigo, $id_proyecto): array
    {
        $data = $json ? json_decode($json) : [];
        $mapaId = [];
        $mapaCodigo = [];

        $currentTable = 'ubicaciones_n' . $level;
        $idUbicacionProperty = 'idUbicacionN' . $level;

        foreach ($data as $nn) {

            $idAgendaReal = $this->obtenerIdAgendaActualizado($nn->idAgenda, $idMapaGeo);

            $previuosLevel = $level - 1;

            //N3
            $prefijo = substr($nn->codigoUbicacion, 0, $previuosLevel * 2);

            $newN_minus_1 = $idMapaN_minus_1_Codigo[$prefijo];

            if ($prefijo !== $newN_minus_1) {
                $prefijo = $newN_minus_1;
            }
            ...
            //Nuevo Código Nn
            $codigoExistente = $codigoExistente ?? '00000000';
            $codigoExistente = str_pad($codigoExistente, $level * 2, '0', STR_PAD_LEFT);
            $codnn = (int)substr($codigoExistente, $previuosLevel * 2, 2) + 1;

            $codnn = str_pad($codnn, 2, '0', STR_PAD_LEFT);

            $nuevoCodigo = $prefijo . $codnn;


            $registro = DB::table($currentTable)
                ->where('idAgenda', $idAgendaReal)
                ->where('idProyecto', $id_proyecto)
                ->where('codigoUbicacion', 'like', $prefijo . '%')
                ->where('descripcionUbicacion', $nn->nombre)
                ->first();

            if (!$registro) {
                $idInsertado = DB::table($currentTable)->insertGetId([
                    'idProyecto'           => $id_proyecto,
                    'idAgenda'             => $idAgendaReal,
                    'codigoUbicacion'      => $nuevoCodigo,
                    'descripcionUbicacion' => $nn->nombre,
                    'estado'               => 1,
                    'fechaCreacion'        => now(),
                    'usuario'              => $usuario,
                    'newApp'               => $nn->newApp,
                ...
                $mapaId[$nn->codigoUbicacion] = $idInsertado;
                $mapaCodigo[$nn->codigoUbicacion] = $nuevoCodigo;
            } else {
                $mapaId[$nn->codigoUbicacion] = $registro->{$idUbicacionProperty};
                $mapaCodigo[$nn->codigoUbicacion] = $registro->codigoUbicacion;
            }
        }
