...
class CiclosEmplazamientosController extends Controller
{
     ...
     * @return  \Illuminate\Http\Response
     */
    public function showAssetsN4(int $ciclo, int $emplazamiento, Request $request)
    {

        $emplaN4Obj = EmplazamientoN4::find($emplazamiento);
        ...
        if ($ciclo != 0) {
            ...
            if ($cicloObj->puntos()->where('idUbicacionGeo', $emplaN4Obj->idAgenda)->count() === 0) {
                return response()->json(['status' => 'error', 'code' => 404, 'message' => 'El emplazamiento nivel  4 no se corresponde con el ciclo'], 404);
            }
        }
    ...
    public function showAssetsByLevel_Legacy(int $ciclo, int $nivel, int $emplazamiento, Request $request)
    {
        switch ($nivel) {
            ...
            case 4:
                return $this->showAssetsN4($ciclo, $emplazamiento, $request);
            default:
     ...
     * @return  \Illuminate\Http\Response
     */
    public function showGroupFamiliesN4(int $ciclo, int $emplazamiento, Request $request)
    {
    ...
    public function showGroupFamiliesByLevel_Legacy(int $ciclo, int $nivel, int $emplazamiento, Request $request)
    {
        switch ($nivel) {
            ...
            case 3:
                return $this->showGroupFamiliesN3($ciclo, $emplazamiento, $request);
            case 4:
                return $this->showGroupFamiliesN4($ciclo, $emplazamiento, $request);
            default:
                return response()->json(['status' => 'error', 'message' => 'Nivel no válido'], 400);
        }
    }
