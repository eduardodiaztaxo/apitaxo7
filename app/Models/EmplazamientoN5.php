...
class EmplazamientoN5 extends Model
{
    use HasFactory;

    protected $table = 'ubicaciones_n5';

    protected $primaryKey = 'idUbicacionN5';
    ...
    {
        return $this->hasMany(CrudActivo::class, 'ubicacionOrganicaN5', 'codigoUbicacion')->where('ubicacionGeografica', $this->idAgenda);
    }
    ...
    {
        return $this->hasMany(Inventario::class, 'codigoUbicacionN5', 'codigoUbicacion')->where('idUbicacionGeo', $this->idAgenda);
    }
    ...
    public function inv_group_families_with_child_levels()
    {
        ...
        $idUbicacionN5 = $this->idUbicacionN5;


        return Inventario::select(
            'ubicaciones_n5.codigoUbicacion',
            DB::raw("'n5' AS place_level"),
            DB::raw("'1' AS isSub"),
        ...
        )->join('ubicaciones_n5', function (JoinClause $join) {

            $join->on('inv_inventario.idUbicacionGeo', '=', 'ubicaciones_n5.idAgenda')
                ->on('inv_inventario.codigoUbicacionN5', '=', 'ubicaciones_n5.codigoUbicacion');
        })
            ...
            ->where('ubicaciones_n5.idUbicacionN5', '=', $idUbicacionN5)
            ->groupBy('ubicaciones_n5.codigoUbicacion', 'inv_inventario.id_ciclo', 'inv_inventario.id_grupo', 'inv_inventario.id_familia', 'dp_grupos.descripcion_grupo', 'dp_familias.descripcion_familia');
    }
    ...
    public function activos_with_cats_by_cycle($cycle_id)
    {
        $queryBuilder = CrudActivo::select('crud_activos.*')->join('inv_ciclos_puntos', 'crud_activos.ubicacionGeografica', 'inv_ciclos_puntos.idPunto')
            ...
            ->where('crud_activos.ubicacionOrganicaN5', '=', $this->codigoUbicacion)
    ...
    {
        return EmplazamientoN4::where('idAgenda', $this->idAgenda)
            ->where('codigoUbicacion', substr($this->codigoUbicacion, 0, 6));
    }

    public function subemplazamientosNivel5()
    {
